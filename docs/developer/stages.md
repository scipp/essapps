...
| Contribute, combine, finalize: the graph up to the accumulation keys, the addition, the graph from the keys to the targets | D15 | `Fold(pipeline, over={member_keys: {accumulation_key: combine}}, outputs=targets)`, whose three entry points are those three |
...
| The fold: a process holding the running contribution and writing a combine record every n arrivals | D15, stateless.md | A `Fold` whose held contributions are combined on demand |
...
The fold holds contributions per member label; `set_members` with one more row costs one contribution, with one row fewer costs a combine over the rest.
That is the sketch's rule for removal, "a combine referencing the remaining contributions, never a subtraction", as the default behavior of the object rather than a rule the framework enforces.
...
That is `Fold.set_members`: the columns identify the group, the index is the member label under which contributions are held.
...
If phase 3 places connectors on process boundaries, that object is the placement and routing layer, and the loops written in phases 1 and 2 say what it must do.
...
The wrapper builds one `Fold` from the spec's accumulation keys and its member keys, the data references among the parameters, and maps between the fold's world and the spec's: a request's parameters to `fold[key] = value` and a member row, and the fold's contribution, keyed by sciline types, to the typed output the spec declares, a data group keyed by field name, and back.
...
- **Memory policy.**
  A `Stage` holds its frontier; a `Fold` holds contributions per member and, as an option, the member frontier, what the graph computes from a member alone, so that a parameter change upstream of the cut does not reload.
  The LoKI validation found the option buys little there: the member frontier lies above the wavelength conversion because that reads a parameter, and a change to the Q bins reruns nearly everything.
  ...
  Judgment: frontier always, contributions in a session, since they are what D15 stores anyway; the member frontier not until a case shows it pays.
...
- D15: name `Fold` as what the wrapper builds from the accumulation keys and the data references, say the associativity helper is generic, say removal is the fold's default, and add the memory policy sentence.
