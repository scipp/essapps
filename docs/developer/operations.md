...
| `cli` | 234 | `essapps serve`, `specs`, `submit`, `wait`, `output` |
...
It holds the raw PIDs the output derives from, the resolved parameters, the spec identity, the package versions, and the environment.
...
**Instrument and proposal are mandatory on every record.**
