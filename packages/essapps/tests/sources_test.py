...
def test_a_pid_is_the_identity_when_the_source_knows_one(tmp_path: Path) -> None:
    ...
    assert dataset.ref == DatasetRef(pid='20.500/abc')
...
def test_locate_finds_the_bytes_of_a_known_identity_only(tmp_path: Path) -> None:
    ...
    assert source.locate(DatasetRef(instrument='dream', run=4711)) == path
    assert source.locate(DatasetRef(instrument='dream', run=1)) is None
    assert source.locate(DatasetRef(pid='20.500/abc')) is None
...
def test_a_file_that_moved_is_located_where_it_is_now(tmp_path: Path) -> None:
    ...
    assert (
        FolderSource(tmp_path).locate(DatasetRef(instrument='dream', run=4711)) is None
    )
