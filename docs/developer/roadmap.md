...
| [Aggregation](aggregation.md) | member requests and a spec over a list of references to all members, for angle series | member and finalize stages with accumulators, for sums over runs | held accumulators; the fold, only if a series arrives faster than its members can be read |
