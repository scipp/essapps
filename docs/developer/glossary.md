...
- **Accumulate**: a stage input whose value is the accumulation of the outputs it lists, `Accumulate(accumulate=[...])`.
...
- **Finalize**: the stage of an aggregation from the accumulated intermediates to the outputs.
  A finalize stage record takes one `Accumulate` per intermediate.
...
- **Input**: a parameter of data-reference type.
  Not the same as a *stage input*, which may also be a literal parameter or an intermediate.
...
- **Intermediate**: an output the spec exposes that a plain run does not compute, a value inside the pipeline such as a numerator or a beam centre.
  A stage may name it as an output, or take it as a stage input in place of what computes it.
  Its value as an input is a reference to an output of a stage record, or an `Accumulate`.
...
- **Member**: one record under a label, identified by its **member key**: a name a person chose for a batch made by hand, the dataset identity for a rule's batch.
  An aggregation's members are the stage records whose intermediates its finalize accumulates.
- **Origin**: the field on a stage request that says where its values came from: the template version, the rule version when a rule filled it, the lookup version and the entry that applied, and the values pinned beyond template and lookup.
...
- **Plain run**: the stage with no inputs, computing the spec's results, as `client.run` and `wf.stage().compute()` submit it.
...
- **Recompute**: an explicit operation that runs a stage record's request again and yields a new record linked to the old one.
...
- **Reference**: a value naming output X of stage record Y, optionally with an element key, or a dataset.
...
- **Series**: the datasets a rule keys together by a metadata value.
  `Series(key, accumulate, outputs)` on a rule makes each arrival submit a member stage request and a finalize stage request.
  See [rules.md](rules.md#series).
- **Session**: a runner plus a private memory cache, belonging to one client, keeping the outputs of its stage records, and the stages and accumulators they name, in memory.
...
- **Workflow**: the scientific code behind a spec: it builds the stage a stage record names, and gives accumulators.
